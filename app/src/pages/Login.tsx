...
export const Login: React.FC<{ onLogin: (r: Role)=>void }> = ({ onLogin }) => {
  ...
  return (
    <Card>
      <h2 className="text-xl font-semibold">Quien eres hoy?</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-lg mt-3">
        <Button variant="secondary" onClick={()=>login('dueno')}>Dueno/Admin</Button>
        <Button variant="secondary" onClick={()=>login('cobranza')}>Cobranza</Button>
        <Button variant="secondary" onClick={()=>login('jefe-tecnico')}>Jefe de Tecnicos</Button>
        <Button variant="secondary" onClick={()=>login('tecnico')}>Tecnico (Sergio)</Button>
